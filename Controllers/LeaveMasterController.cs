...
namespace Leave_Management_System.Controllers
{
    ...
    public class LeaveMasterController : BaseController
    {
        ...
        public LeaveMasterController(ApplicationDbContext db)
        {
            _db = db;
        }
        ...
        public IActionResult List()
        {
            ...
            if (!string.IsNullOrEmpty(searchValue))
            {
                ...
                query = query.Where(x =>
                    x.LeaveName.ToLower().Contains(searchValue) ||
        ...
        public async Task<IActionResult> Edit(string id)
        {
            ...
            var LeaveMaster = await _db.LeaveMaster.FindAsync(decryptedId);
        ...
        public async Task<IActionResult> Edit(LeaveMaster model)
        {
            ...
            try
            {
                ...
                var existing = await _db.LeaveMaster.FindAsync(model.Id);
                ...
                return Json(new { success = true, message = "Leave Master updated successfully" });
        ...
        public async Task<IActionResult> Details(string id)
        {
            ...
            var LeaveMaster = await _db.LeaveMaster
                .FirstOrDefaultAsync(x => x.Id == decryptedId);

            if (LeaveMaster == null)
                return NotFound();

            var createdByUser = await _db.Users.FirstOrDefaultAsync(m => m.Id == LeaveMaster.CreatedBy);
            var updatedByUser = await _db.Users.FirstOrDefaultAsync(m => m.Id == LeaveMaster.UpdatedBy);
        ...
        public async Task<IActionResult> Delete(string id)
        {
            ...
            try
            {
                ...
                var LeaveMaster = await _db.LeaveMaster.FindAsync(decryptedId);
                if (LeaveMaster == null)
                    return Json(new { success = false, message = "Leave Master not found" });

                _db.LeaveMaster.Remove(LeaveMaster);
                await _db.SaveChangesAsync();

                return Json(new { success = true, message = "Leave Master deleted successfully" });
            }
            catch (Exception ex)
            {
